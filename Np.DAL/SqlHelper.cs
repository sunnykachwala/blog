namespace Np.DAL
{
    ...
    public class SqlHelper : ISqlHelper
    {
        ...
        public SqlHelper(NewsPortalContext context)
        {
            this.context = context;
        }

        /// <summary>
        /// This method is used to get connection string value from web.config file.        
        /// </summary>
        /// <param name="void">Void parameter</param>
        public string GetConnectionString()
        {
            return this.context.Database.GetConnectionString();
        ...
        public int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText)
        {
            ...
            return ExecuteNonQuery(connectionString, commandType, commandText, commandParameters: param);
        }
        ...
        public int ExecuteNonQuery(SqlConnection connection, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            ...
            if (connection.ConnectionTimeout > cmd.CommandTimeout)
		...
		public int ExecuteNonQuery(SqlTransaction transaction, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            ...
            if (transaction.Connection.ConnectionTimeout > cmd.CommandTimeout)
        ...
        public int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText, SqlTransaction transaction)
        {
            int NonQuery = 0;
        ...
        public int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText, SqlTransaction transaction, string[] parameters, object[] values)
        {
            int NonQuery = 0;
            using (SqlConnection con = new SqlConnection(connectionString))
            {
        ...
        public int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText, string[] parameters, object[] values)
        {
            int NonQuery = 0;
            using (SqlConnection con = new SqlConnection(connectionString))
            {
        ...
        public DataSet ExecuteDataset(SqlConnection connection, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            ...
            // Create a command and prepare it for execution
            SqlCommand cmd = new SqlCommand();
            //cmd.CommandTimeout = int.Parse(System.Configuration.ConfigurationManager.AppSettings["ConnTimeOut"]);
            bool mustCloseConnection = false;
            PrepareCommand(cmd, connection, (SqlTransaction)null, commandType, commandText, commandParameters, out mustCloseConnection);
            if (connection.ConnectionTimeout > cmd.CommandTimeout)
                cmd.CommandTimeout = connection.ConnectionTimeout;
        ...
        public DataTable ExecuteDataTableOdbc(string connectionString, CommandType commandType, string commandText)
        {
            using (OdbcConnection con = new OdbcConnection(connectionString))
            {
                ...
                {
                    con.Open();
                    using (OdbcCommand cmd = new OdbcCommand(commandText, con))
                    {
                        cmd.CommandType = commandType;
        ...
        public OdbcDataReader ExecuteDataReaderOdbc(string connectionString, CommandType commandType, string commandText)
        {
            using (OdbcConnection con = new OdbcConnection(connectionString))
            {
                try
        ...
        public int ExecuteNonQueryOdbc(string connectionString, CommandType commandType, string commandText, OdbcTransaction transaction)
        {
            int NonQuery = 0;
        ...
        #region Create Dynamic DataTable

        #region Simple Creation

        public DataTable CreateDataTable(string primary_Key, string[] parameters, string tblName)
        {
            try
            {
                DataTable dt = new DataTable(tblName);
                dt.Columns.Add(primary_Key, typeof(System.Int64));
                dt.Columns[primary_Key].AutoIncrement = true;
                dt.Columns[primary_Key].AutoIncrementSeed = 1;

                if (parameters.Length > 0)
                {
                    for (int i = 0; i < parameters.Length; i++)
                    {
                        dt.Columns.Add(parameters[i]);
                    }
                }
                return dt;
            }
        ...
        public DataTable CreateDataTable(string primary_Key, string[] parameters)
        {
            try
            {
                DataTable dt = new DataTable();
        ...
        #region Create then Bind Dynamic DataTable with Values

        //Note: Array length must be equal of parameters & values

        public DataTable CreateDataTable(DataTable dtt, string primary_Key, string[] parameters, object[] values, string tblName)
        {
            try
            {
                bool dtDone = false;
                ...
                if (dtDone == false)
                {
                    dtt = new DataTable(tblName);
                    dtt.Columns.Add(primary_Key, typeof(System.Int64));
                    dtt.Columns[primary_Key].AutoIncrement = true;
                    dtt.Columns[primary_Key].AutoIncrementSeed = 1;

                    if (parameters.Length > 0)
                    {
                        for (int i = 0; i < parameters.Length; i++)
                        {
                            dtt.Columns.Add(parameters[i]);
                        }
                    }
                    ...
                    {
                        for (int i = 0; i < (dtt.Columns.Count - 1); i++)
                        {
                            dr[parameters[i]] = values[i];
                        }
                    }
                    dtt.Rows.Add(dr);
                    dtDone = true;
        ...
        public DataTable CreateDataTable(DataTable dtt, string primary_Key, string[] parameters, object[] values)
        {
            try
            {
                bool dtDone = false;
    ...
    /// <summary>
    /// SqlHelperParameterCache provides functions to leverage a   cache of procedure parameters, and the
